...
def save_log(name, results, idn=None):
    fname = f"doom3_log_{name}_{datetime.now():%Y%m%d_%H%M%S}.txt"
...
def main():
    if len(sys.argv) < 2:
        ...
        print("====================================")
        print("  🔍 doom3 list                            List all VISA resources")
        print("  🧪 doom3 test      --ip <addr> | --usb   Run full SCPI test suite")
        print("  🎯 doom3 group     <GROUP> --ip | --usb  Test SCPI commands by group (e.g., MATH1)")
        print("  🧾 doom3 licenses  <ip>                  Query installed license keys")
        print("  📉 doom3 waveform  <CH> --ip | --usb     Retrieve waveform data (e.g., CHAN1)")
        print("  💣 doom3 fuzz      --ip <addr> | --usb   Fuzz scope with random SCPI queries")
        print("  🧠 doom3 learn     --ip <addr> | --usb   Discover new SCPI commands via probing")
        print("  ✉️ doom3 send \"<SCPI>\" --ip | --usb    Send any SCPI command (quoted)")
        print("  🐰 doom3 pinky                          Activate Gehirnwäsche mode (easter egg)")
        print("====================================")
        if DRY_RUN:
            print("🚫  NOTE: --dry-run mode is enabled — no SCPI commands will be sent!")
        print("🛠️  Example: doom3 test --ip 192.168.2.70")
        return
